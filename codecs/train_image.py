...
PARQUET_FILES: List[str] = [
    ...
    "/home/astrodados4/downloads/hypercube/datacube_HYDRA-0013.parquet",
    "/home/astrodados4/downloads/hypercube/datacube_HYDRA-0014.parquet",
    "/home/astrodados4/downloads/hypercube/datacube_SPLUS-n18s07.parquet",
    "/home/astrodados4/downloads/hypercube/datacube_SPLUS-n18s11.parquet",
    "/home/astrodados4/downloads/hypercube/datacube_SPLUS-s24s13.parquet",
]
...
LATENT_DIM = 2            # per-position latent dimension
BATCH_SIZE = 64
...
USE_SIGMA_IN_LOSS = True # keep False for now
...
def visualize_epoch(
    ...
    """
    Visualize random objects from a DataFrame already in memory.

    - df: polars DataFrame with S-PLUS cuts and geometry columns
    - n_samples: number of random objects to plot (default=10)
    """
    ...
    if df.height == 0:
        print("Visualization: DataFrame is empty, skipping.")
        return

    # number of objects to plot
    N_vis = min(n_samples, df.height)

    # random indices instead of first N
    idxs = np.random.choice(df.height, size=N_vis, replace=False)

    # reuse dataset logic but with your in-memory df
    ds = SplusDataset(df)

    xs = []
    m_objs = []
    for idx in idxs:
        x_norm, m_obj, _ = ds[int(idx)]
        xs.append(x_norm)
        m_objs.append(m_obj)
    ...
    for row in range(N_vis):
        ...
        im0 = ax0.imshow(img_orig, origin="lower", cmap="gray",
                         vmin=vmin, vmax=vmax)
        ax0.set_title(f"Original norm ({BANDS[band_idx]}) row={row}")
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Use the DataFrame already held by the first loader's dataset
        first_loader = loaders_info[0][2]
        df_vis = first_loader.dataset.df  # polars DataFrame already in memory
        visualize_epoch(model, device, epoch + 1, df_vis, n_samples=10)

    out_path = "autoencoder_splus_cuts.pt"
